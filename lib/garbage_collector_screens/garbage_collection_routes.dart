...
class _GarbageCollectionRoutesState extends State<GarbageCollectionRoutes> {
  ...
  void initState() {
    super.initState();
    _getCollectorLocation();
  }

  Future<void> _getCollectorLocation() async {
    // Assuming collector's location is stored in the 'users' collection with a specific ID
    final String collectorId =
        'LA2x3Re3ZAYwjhyYHxwmZAYeOuY2'; // Replace with the actual ID
    ...
    if (collectorDoc.exists) {
      GeoPoint location = collectorDoc['location'];
      setState(() {
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => UpdateCollectorLocationScreen(),
                ),
              );
            },
          ),
          ...
          : StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                snapshot.data!.docs.forEach((doc) {
                  ...
                  if (distance <= 10) {
                    // Filter routes within 10 km radius
                    if (isCompleted) {
